...
function eventsCallback(snapshot) {
     ...
     if (events === undefined) {
      ...
      $.each(events, function(i){
            ...
            eventInList+= "<h2 class='list standard-inset' id="+i+">what the fk"+events[i].title+"</h2>"
        //    eventInList+= "<p class='detail standard-inset'>"+events[i].content+"</p></br></div>"
            eventInList+= "</p></br></div>"
      });
      $('#events_main').empty();
      ...
      $('.event_item').click(function(){
        ...
        if (ev.location_1 !== undefined) {
          ...
          eventDetail+= '<button type="button" class="event_b">VIEW ON MAP</button> <hr>'
        }
